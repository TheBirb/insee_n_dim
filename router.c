...
void init_network(void) {
	...
	if (topo < DIRECT){
		...
		create_graph();
	else if (topo==DRAGONFLY_ABSOLUTE || topo==DRAGONFLY_RELATIVE || topo==DRAGONFLY_CIRCULANT || topo==DRAGONFLY_HELIX || topo==DRAGONFLY_NAUTILUS || topo==DRAGONFLY_OTHER)
		create_dragonfly();
...
void router_finish(){
	...
	for(i = 0; i < NUMNODES; ++i) {
		...
		if (i<nprocs) { // Injection queues only in processors
			for (j=0; j<ninj; j++)
                            free(network[i].qi[j].pos);
                        free(network[i].qi);
		}

		for(j = 0; j < n_ports+1; ++j) {
                    free(network[i].p[j].req);
                    free(network[i].p[j].histo);
                    free(network[i].p[j].q.pos);
		}
                free(network[i].p);

                free(network[i].op_i);
                free(network[i].nbor);
                free(network[i].nborp);


#if (TRACE_SUPPORT > 1)
		if (i<nprocs){
                        free(network[i].occurs);
		}
#endif
	}
...
void finish_network(){
    ...
    if (topo == RRG || topo == EXA || topo == GDBG || topo == KAUTZ)
        finish_graph();
    if (topo==DRAGONFLY_ABSOLUTE || topo==DRAGONFLY_RELATIVE || topo==DRAGONFLY_CIRCULANT || topo==DRAGONFLY_HELIX || topo==DRAGONFLY_NAUTILUS || topo==DRAGONFLY_OTHER){
        finish_dragonfly();
