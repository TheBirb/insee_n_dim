...
//	}
     ...
     if (!drop_packets && network[i].pending_packet > 0){
		...
		case SHIFT:
		case BISECT:
			d = (next_dest[i])%nprocs;// allows for non 2^k topologies
			if (i == d){
				return ;
			}
			break;
		...
		case GROUPSHIFT:
			do {
				d = ((((i/group_size)+stride)*group_size)+(long)(1.0*group_size*rand()/(RAND_MAX+1.0)))%nprocs;
			} while (d == i);
			break;
		case POPULATION:
...
void init_injection	(void) {
	...
	shuf = alloc(sizeof(long)*nprocs);
	scount = 0;


    if (pattern==POPULATION)
		read_population();
	else if (pattern==HISTOGRAM)
		read_histogram();
	// Traces are read from run_network_trc()
	...
	for (i=0; i<nprocs; i++) {
		...
		switch (pattern) {
			...
			case HOTREGION:
				...
				break;
			case ADV:
				// From 'Technology-Driven, Highly-Scalable Dragonfly Topology'
				stride=1;
				group_size=param_p*param_a;
				if (topo!=DRAGONFLY_ABSOLUTE && topo!=DRAGONFLY_CIRCULANT && topo!=DRAGONFLY_RELATIVE && topo!=DRAGONFLY_NAUTILUS && topo!=DRAGONFLY_HELIX && topo!=DRAGONFLY_OTHER)
					panic("ADV only defined for Dragonfly topologies; try groupshift instead");
				break;
			case GROUPSHIFT:
			case UNIFORM:
			case SEMI:
			...
			case LOCAL:
				if (topo!=TORUS || topo!=SPINNAKER)
					printf("WARNING: Not a toroidal topology, LOCAL traffic may not work as intended!\n");
				if (nodes_x<16)
					printf("WARNING: nodes_x<16, LOCAL traffic may not work as intended!\n");
				if (ndim>=2 && nodes_y<16)
					printf("WARNING: nodes_y<16, LOCAL traffic may not work as intended!\n");
				if (ndim>=3 && nodes_z<16)
					printf("WARNING: nodes_z<16, LOCAL traffic may not work as intended!\n");
				break;
			case BISECT:
				shuf[i] = i;
				break;
	...
	if (pattern==BISECT){ // As defined in Xin Yuan, Santosh Mahapatra, Wickus Nienaber, Scott Pakin, and Michael Lang. 2013. A new routing scheme for Jellyfish and its performance with HPC workloads. In Proceedings of the International Conference on High Performance Computing, Networking, Storage and Analysis (SC '13)
		...
		for (i=0; i<nprocs; i++){
			next_dest[shuf[i]]=shuf[nprocs-i-1]; //each randomly shuffled node i gets paired with nprocs-i. A more elegant way of dealing with odd number of nodes.
		}
	}
	if (shotmode)
